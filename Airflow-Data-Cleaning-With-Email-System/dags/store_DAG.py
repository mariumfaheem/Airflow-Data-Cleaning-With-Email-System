...
with DAG('store_dag',
    ...
    t6=BashOperator(task_id="move_file_1",
    ...
    t8=EmailOperator(task_id="send_email",
                     ...
                     html_content="""<h1>Congratulation! Your store report are ready</h1>""",
                     files=['usr/local//airflow/location_wise_profit_%s.csv' %yesterday_date, '/usr/local/airflow/store_files_airflow/store_wise_profit_%s.csv' %yesterday_date]
                     )

    t9=BashOperator(task_id="rename_raw",
                    bash_command='mv ~/store_filess_airflow/raw_store_transactions.csv ~/store_filess_airflow/raw_store_transactions_%s.csv ' %yesterday_date
                    )
